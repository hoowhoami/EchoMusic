...
const PREFETCH_TARGET_BYTES: u64 = 10 * 1024 * 1024;
...
impl HttpRangeInput {
    ...
    pub fn seek(&mut self, offset: i64, whence: i32) -> Result<i64, HttpRangeError> {
        ...
        self.position = (next as u64).min(len);
...
impl HttpRangeShared {
    fn read_at(self: &Arc<Self>, position: u64, out: &mut [u8]) -> Result<usize, HttpRangeError> {
        loop {
            ...
            self.ensure_fetch_locked(&mut state, position);
    ...
    fn seek_to(self: &Arc<Self>, position: u64) {
        if let Ok(mut state) = self.state.lock() {
            ...
            // Clear in-flight ranges that are now irrelevant
            let new_generation = state.generation;
            state.in_flight.retain(|range| {
                let range_end = range.end.saturating_add(1);
                let relevant = position >= range.start && position < range_end
                    || range.start >= position && range.start < position.saturating_add(PREFETCH_TARGET_BYTES);
                relevant && range.generation == new_generation
            });
    ...
    fn ensure_readahead_locked(self: &Arc<Self>, state: &mut HttpRangeState, position: u64) {
        ...
        while cursor < target && state.in_flight.len() < MAX_IN_FLIGHT {
            ...
            spawn_fetch(Arc::clone(self), start, end, seq, generation);
    ...
    fn ensure_fetch_locked(self: &Arc<Self>, state: &mut HttpRangeState, position: u64) {
...
fn spawn_fetch(shared: Arc<HttpRangeShared>, start: u64, end: u64, seq: u64, generation: u64) {
    let _ = thread::Builder::new()
        .name("echo-http-range-fetch".to_string())
        .spawn(move || {
            let result = fetch_range_with_retries(&shared, start, end, generation);
            if let Ok(mut state) = shared.state.lock() {
                ...
                match result {
                    ...
                    FetchRangeResult::Failed(message) if state.generation == generation => {
...
fn fetch_range_with_retries(
    ...
    for (attempt, backoff) in FETCH_RETRY_BACKOFFS.iter().enumerate() {
        ...
        match fetch_range(shared, next_start, end, generation) {
...
fn fetch_range(
    ...
    if fetch_is_stale(shared, generation) {
...
fn fetch_is_stale(shared: &HttpRangeShared, generation: u64) -> bool {
...
fn cached_or_in_flight_contiguous_end(state: &HttpRangeState, position: u64) -> u64 {
    ...
    loop {
        ...
        for range in &state.cache {
            let end = range.start.saturating_add(range.data.len() as u64);
            if range.start <= cursor && cursor < end && end > cursor {
                cursor = end;
                advanced = true;
            }
        }
...
fn trim_cache(state: &mut HttpRangeState) {
    while state.cached_bytes > MAX_CACHE_BYTES {
...
#[cfg(test)]
mod tests {
    use super::{insert_cache_range, parse_content_range_start_value, parse_content_range_total_value};
