...
import { allocateAudioQueueMemory, createMainController, type MainAudioController } from './queue';
import {
  type EngineConfig,
  type EngineError,
  EngineErrorCode,
...
export class FFmpegAudioEngine extends TypedEventTarget<EngineEventMap> {
    ...
    this.preloadPromise = (async () => {
      try {
        const [ffmpegWasmBuffer, stWasmBuffer] = await Promise.all([
          ...
          this.fetchAndValidateWasm(this.config.assets.soundtouchWasmUrl, signal),
  ...
  public async loadFile(file: File): Promise<void> {
    ...
    if (!this.ffmpegWasmBlobUrl || !this.soundtouchWasmBuffer) {
      const msg = 'Assets are missing even after preload was called. Engine cannot proceed.';
    ...
    try {
      ...
      this.workerClient.init(file, sampleRate, channels, this.sharedBuffer, this.ffmpegWasmBlobUrl);
