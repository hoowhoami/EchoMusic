...
async function processFrame() {
  ...
  try {
    const { async, promise } = audioWriter.waitForSpaceAsync(WORKER_MIN_SPACE_REQ_FRAMES);
    ...
    if (async && promise) {
      ...
      if (status === 1) {
        const samples = ffmpegModule._wasm_decoder_get_frame_samples(decoderPtr);
        ...
        while (written < samples) {
          ...
          const pushed = audioWriter.writePartial(channelDatas, written, remaining);
          written += pushed;
    ...
    if (myGeneration !== currentSeekGeneration && mySessionId === currentSessionId && isDecoding) {
      processFrame();
    }
  }

  if (isDecoding && myGeneration === currentSeekGeneration && mySessionId === currentSessionId) {
...
self.onmessage = async (e: MessageEvent<WorkerCommand>) => {
  ...
  if (data.type === 'INIT') {
    ...
    try {
      ...
      decoderPtr = ffmpegModule._wasm_decoder_create(1, targetSampleRate, targetChannels);

      if (decoderPtr === 0) {
        throw new Error(`Decoder Context creation failed: ${getLastErrorMsg()}`);
      }

      const duration = ffmpegModule._wasm_decoder_get_duration(decoderPtr);
      const metadataJsonPtr = ffmpegModule._wasm_decoder_get_metadata_json(decoderPtr);
    ...
    const seekStatus = ffmpegModule._wasm_decoder_seek(decoderPtr, data.payload.targetSeconds);
    audioWriter.endSeek();
