...
export interface AudioWriter {
   ...
   */
  writePartial(channelDatas: Float32Array[], offset: number, length: number): number;
...
class AudioQueueCore implements MainAudioController, AudioWriter, AudioReader {
  ...
  constructor(sab: SharedArrayBuffer) {
    ...
    this.capacityFrames = Atomics.load(this.controlBlock, CONFIG_CAPACITY_FRAMES);

    const bytesPerChannel = this.capacityFrames * 4;
    for (let c = 0; c < this.channels; c++) {
      const offset = CONTROL_BLOCK_BYTES + c * bytesPerChannel;
      this.channelBuffers.push(new Float32Array(sab, offset, this.capacityFrames));
    }
  }
  ...
  writePartial(channelDatas: Float32Array[], offset: number, length: number): number {
    ...
    for (let c = 0; c < this.channels; c++) {
      ...
      if (writeAmount <= spaceToEnd) {
        ...
        target.set(source.subarray(offset + spaceToEnd, offset + writeAmount), 0);
      }
    }
  ...
  waitForSpaceAsync(minSpaceReq: number): {
    ...
    const waitResult = Atomics.waitAsync(this.controlBlock, STATE_READ_INDEX, currentReadIndex);
  ...
  waitForDrainAsync(): { isDrained: boolean; promise?: Promise<void> } {
    ...
    const waitResult = Atomics.waitAsync(this.controlBlock, STATE_READ_INDEX, readIndex);
  ...
  private notifyProducerIfNeeded(availableData: number): void {
    ...
    const emergencyFrames = Atomics.load(this.controlBlock, CONFIG_EMERGENCY_FRAMES);
...
export function allocateAudioQueueMemory(
  ...
  const capacityFrames = Math.ceil((config.capacitySeconds * sampleRate) / 128) * 128;
  const notifyFrames = Math.round(config.notifyWatermarkSeconds * sampleRate);
  const emergencyFrames = Math.round(config.emergencyWatermarkSeconds * sampleRate);
 ...
 */
export function createMainController(sab: SharedArrayBuffer): MainAudioController {
