...
export class AudioRenderer {
  ...
  public bindQueue(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.workletNode.port.onmessage = (event: MessageEvent<WorkletEvent>) => {
        ...
        if (data.type === 'INIT_DONE' && data.payload.initId === currentInitId) {
          resolve();
        } else if (data.type === 'INIT_ERROR' && data.payload.initId === currentInitId) {
  ...
  public async resumeContext(): Promise<void> {
    if (!this._isWorkletLoaded) {
      console.warn('AudioRenderer: Context resumed before Worklet initialization.');
    }
