...
import initWasm, { SoundTouchProcessor, StretchAlgorithm } from './wasm/soundtouch';
...
class FFmpegAudioProcessor extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    this.port.onmessage = async (event: MessageEvent<WorkletCommand>) => {
      ...
      if (data.type === 'INIT') {
        ...
        const { sharedBuffer, channels, wasmBytes, initId, tempo, pitch, rate, algorithm } =
          data.payload;
        ...
        try {
          ...
          this.stProcessor = new SoundTouchProcessor(channels, sampleRate, algoEnum);
        ...
        this.stProcessor.setFormantFactor(data.payload.factor, data.payload.compensatePitch);
  ...
  process(
    ...
    if (targetPauseIndex !== -1) {
      const exactCurrentIndex = this.audioReader.getPlaybackIndex() + this.playbackFractional;
    ...
    while (this.stProcessor.numSamples() < WORKLET_BLOCK_SIZE) {
      ...
      for (let c = 0; c < this.channels; c++) {
        ...
        wasmInputs.push(new Float32Array(this.wasmMemory.buffer, ptr, readAmount));
      }
