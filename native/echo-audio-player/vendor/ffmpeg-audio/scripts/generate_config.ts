...
if (targetOs === 'windows') {
  ...
  if (targetArch === 'x86_64') {
    ...
    options.push('--target-os=win32', '--arch=aarch64', '--enable-cross-compile');
    targetArch = 'arm64';
  } else {
  ...
  if (targetArch === 'arm64' || targetArch === 'aarch64') {
    options.push('--arch=aarch64', '--enable-cross-compile', '--cc=aarch64-linux-gnu-gcc');
    targetArch = 'arm64';
  } else if (targetArch === 'armv7') {
    options.push('--arch=arm', '--enable-cross-compile', '--cc=arm-linux-gnueabihf-gcc');
  } else if (targetArch === 'x86') {
    options.push('--arch=x86', '--enable-cross-compile', '--cc=i686-linux-gnu-gcc');
  ...
  const ndkPath = process.env.ANDROID_NDK_LATEST_HOME || process.env.ANDROID_NDK_HOME;
  if (!ndkPath) {
  ...
  if (targetArch === 'arm64' || targetArch === 'aarch64') {
    ...
    const xcrunResult = spawnSync('xcrun', ['--sdk', 'iphoneos', '--show-sdk-path']);
  ...
  options.push('--disable-pthreads', '--disable-w32threads', '--disable-os2threads');
...
const configureResult = spawnSync('bash', ['-c', `../configure ${safeOptions}`], {
  cwd: buildDir,
  stdio: 'inherit',
});
...
if (configureResult.status === 0) {
  ...
  const makeResult = spawnSync('bash', ['-c', 'make V=1 -n > make_dryrun.log'], {
    cwd: buildDir,
    stdio: 'inherit',
  });
