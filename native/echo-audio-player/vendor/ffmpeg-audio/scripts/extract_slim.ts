...
function parseLog(logPath: string): {
  ...
  for (const line of content.split(/\r?\n/)) {
    if ((!line.includes('-c -o ') && !line.includes('-c -Fo')) || !line.includes('.c')) {
      continue;
    }
...
if (fs.existsSync(configsDir)) {
  for (const entry of fs.readdirSync(configsDir, { withFileTypes: true })) {
    ...
    console.log(`  ${entry.name}: ${cFiles.size} C 文件, ${includeDirs.size} -I 路径`);
  }
}
...
while (true) {
  ...
  for (const filePath of cFiles) {
    try {
      ...
      for (const match of content.matchAll(includeCPattern)) {
        ...
        const candidate = path.normalize(path.join(path.dirname(filePath), included));

        try {
          const relToSlimDst = path.relative(path.normalize(slimDst), candidate);
  ...
  for (const relStr of Array.from(newlyNeeded).sort()) {
    ...
    if (dst) {
      ...
      console.log(`  !! 被 include 的 C 文件缺失: ${path.join(ffmpegSrc, relStr)}`);
    }
  }
}
...
for (const rootRel of Array.from(hScanRoots).sort()) {
  ...
  for (const filePath of hFiles) {
    try {
      const relToSrc = path.relative(path.normalize(ffmpegSrc), path.normalize(filePath));
      const dst = path.join(slimDst, relToSrc);
...
await createZip(slimDst, path.join('crates', 'ffmpeg_audio_sys', 'vendor', 'ffmpeg_slim.zip'));
await createZip(configsDir, path.join('crates', 'ffmpeg_audio_sys', 'vendor', 'configs.zip'));
