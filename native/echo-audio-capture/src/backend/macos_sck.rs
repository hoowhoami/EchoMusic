...
pub fn start(ring: Arc<Mutex<SampleRing>>) -> Result<MacSckSession, String> {
    let shareable_content_class = class("SCShareableContent").map_err(|_| {
        "ScreenCaptureKit system audio capture requires macOS 12.3 or later".to_string()
    })?;
    ...
    ring.lock()
        ...
        .configure(DEFAULT_SAMPLE_RATE, 2)?;
...
fn push_f64_pcm(ring: &mut SampleRing, data: &[u8], channels: usize, bytes_per_frame: usize) {
    for frame in data.chunks_exact(bytes_per_frame) {
        for channel in 0..channels {
            ...
            ring.push(f64::from_ne_bytes(frame[offset..offset + 8].try_into().unwrap()) as f32);
        }
    }
}
...
fn push_i16_pcm(ring: &mut SampleRing, data: &[u8], channels: usize, bytes_per_frame: usize) {
    for frame in data.chunks_exact(bytes_per_frame) {
        for channel in 0..channels {
            ...
            ring.push(
                i16::from_ne_bytes(frame[offset..offset + 2].try_into().unwrap()) as f32
                    / i16::MAX as f32,
            );
        }
    }
...
fn push_i32_pcm(ring: &mut SampleRing, data: &[u8], channels: usize, bytes_per_frame: usize) {
    for frame in data.chunks_exact(bytes_per_frame) {
        for channel in 0..channels {
            ...
            ring.push(
                i32::from_ne_bytes(frame[offset..offset + 4].try_into().unwrap()) as f32
                    / i32::MAX as f32,
            );
        }
    }
