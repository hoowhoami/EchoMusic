...
// 歌单数据
const playlistData = shallowRef<Song[]>([]);
const playlistDetailData = ref<Playlist | null>(null);
...
const songListHeight = computed(() => {
  return 400 - (listScrolling.value ? 120 : 240);
});

// 是否为用户歌单
const isUserPlaylist = computed(() => {
  return playlistDetailData.value?.list_create_userid === userStore.userid;
});
...
// 获取歌单基础信息
const getPlaylistDetailInfo = async (
  id: string,
...
// 获取在线歌单
const handleOnlinePlaylist = async (id: string, getList: boolean, refresh: boolean) => {
...
onActivated(() => {
  ...
  if (oldPlaylistId.value === 0) {
    ...
    oldPlaylistId.value = playlistId.value;
    // 刷新歌单
    getPlaylistDetailInfo(playlistId.value, { getList: true, refresh: isSame });
  }
});

onDeactivated(() => loadingMsgShow(false));
onUnmounted(() => loadingMsgShow(false));
onMounted(() => getPlaylistDetailInfo(playlistId.value));
</script>
