...
const source = readFileSync(
  new URL('../src/renderer/miniPlayer/MiniPlayerView.vue', import.meta.url),
  'utf8',
);
...
const reconcile = new Function(
  ...
  'pendingVolumeDeadline',
  `${code}; return { nextPlayback, pendingVolumeRequest };`,
);
test('old snapshots cannot rewind optimistic volume; latest acknowledgement releases it', () => {
  ...
  const old = reconcile(
    { playback: { volume: 55, volumeRequestId: 'old', currentTime: 10 } },
    local,
    'latest',
    deadline,
  );
  ...
  const ack = reconcile(
    { playback: { volume: 65, volumeRequestId: 'latest' } },
    local,
    'latest',
    deadline,
  );
...
test('wheel at zero does not restore old volume', () => {
  ...
  const adjust = new Function(
    ...
    'setVolume',
    `${code}; return adjustVolume;`,
  )(
    { value: { volume: 0, lastNonZeroVolume: 75 } },
