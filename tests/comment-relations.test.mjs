...
const source = buildSync({
  entryPoints: [new URL('../src/renderer/utils/commentRelations.ts', import.meta.url).pathname],
  bundle: true,
  format: 'cjs',
  platform: 'node',
  write: false,
...
const floor = {
  id: '774320303',
  content: '111',
  raw: { tid: 14270239, pid: 0, ori_cmt_id: 14270249 },
};
...
test('ordinary content and malformed quotes stay intact; nested references remain in quoted text', () => {
  ...
  assert.deepEqual(presentFloorReply({ content, raw: {} }, []), {
    body: content,
    quote: undefined,
  });
  assert.equal(
    presentFloorReply({ content: '333//@name', raw: { pid: 1 } }, []).body,
    '333//@name',
  );
  assert.deepEqual(
    presentFloorReply({ content: '333//@name:111\n//@other:222', raw: { pid: 1 } }, []),
    {
      body: '333',
      quote: { userName: 'name', content: '111\n//@other:222' },
    },
  );
});
...
test('singer reply uses explicit thread tid rather than the quoted reply pid', () => {
  const singer = {
    id: '1060329201',
    content: '远山少年原唱是我～//@佳的太阳:黄文文的远山少年吗',
    raw: { pid: 732952861, tid: 1690487954 },
  };
  ...
  assert.deepEqual(presentFloorReply(singer, []), {
    body: '远山少年原唱是我～',
    quote: { userName: '佳的太阳', content: '黄文文的远山少年吗' },
  });
});
