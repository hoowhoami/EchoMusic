...
const source = buildSync({
  entryPoints: [new URL('../src/renderer/utils/barrage.ts', import.meta.url).pathname],
  bundle: true,
  format: 'cjs',
  platform: 'node',
  write: false,
...
test('ownership preserves user identity and excludes anonymous and invalid IDs', () => {
  ...
  for (const value of [null, undefined, 0, '0', '', 'undefined', {}, -1])
    assert.equal(normalizeBarrageUserId(value), '');
  assert.deepEqual(
    normalizeBarrageItems([{ content: ' hello ', user_id: '123' }, null, { content: ' ' }]),
    [{ text: 'hello', userId: '123' }],
  );
});
test('list size is bounded independently of the upstream response size', () => {
  assert.equal(
    normalizeBarrageItems(Array.from({ length: 1000 }, () => ({ content: 'test' }))).length,
    100,
  );
});
...
test('local sends suppress immediate server echoes but not other users or later playback', () => {
  ...
  const own = { text: 'hello', userId: '1' },
    other = { text: 'hello', userId: '2' };
  ...
  for (const width of [320, 1000, 1920]) {
    for (const text of [40, 300, 4000]) {
      for (const speed of [0.5, 1, 2]) {
        assert.ok(
          Math.abs(
            (width + text) / barrageTravelDuration(width, text, speed) - BARRAGE_BASE_SPEED * speed,
          ) < 1e-8,
        );
      }
    }
