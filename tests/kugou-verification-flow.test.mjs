...
test('v_type 51 requires an APP appeal and cannot be submitted as a captcha', async () => {
  for (const v_type of [51, '51']) {
    ...
    await tick();
    assert.equal(
      api.getKugouCaptchaProvider(api.kugouVerificationState.verifyInfo),
      'ACCOUNT_RISK',
    );
...
test('expired event can be cancelled and a new request gets a fresh challenge', async () => {
  const api = setup();
  const first = api.requestKugouVerification('expired', async () => {
    throw { response: { body: { error_code: 36001 } } };
  });
  ...
  api.cancelKugouVerification();
  await rejected;
  const second = api.requestKugouVerification('new', async () => ({
    status: 1,
    data: { v_type: 23 },
  }));
  await tick();
...
test('late failure of a cancelled challenge cannot corrupt the next challenge', async () => {
  const api = setup();
  let rejectOld;
  const first = api.requestKugouVerification(
    'old',
    () =>
