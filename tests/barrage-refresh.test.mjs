...
const source = readFileSync(
  new URL('../src/renderer/components/music/BarrageLayer.vue', import.meta.url),
  'utf8',
);
...
const a = { text: 'a', userId: '1' },
  b = { text: 'b', userId: '2' },
  c = { text: 'c', userId: '3' };
function setup(getBarrage) {
  const state = {
    items: { value: [a, b] },
    flights: { value: [{ ...a, id: 1, lane: 0 }] },
    loading: { value: false },
    error: { value: '' },
  };
...
test('successful send waits for a free lane without replacing existing flights', () => {
  ...
  const handlers = transformSync(source.slice(start, source.indexOf('watch(', start)), {
    loader: 'ts',
  }).code;
  const original = Array.from({ length: 4 }, (_, lane) => ({
    id: lane + 1,
    lane,
    text: 'old',
    userId: '2',
  }));
  const flights = { value: [...original] },
    pendingOwn = { value: [] },
    recentOwn = new Map();
  const run = new Function(
    'flights',
