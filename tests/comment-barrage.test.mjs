...
test('all five send routes pass resource identity in POST body', async () => {
  ...
  for (const [, body] of calls.slice(3)) {
...
test('business failures and HTTP failures do not report a successful send', async () => {
  ...
  const api = client({
    post: async () => {
      throw { response: { body: { msg: '需要手机验证' } } };
    },
  });
...
test('song comments, song barrage and MV barrage resolve distinct pools before sending', async () => {
  for (const [name, params, expectedCode, expectedAction] of [
    ...
    const calls = [];
    await handler(
      { ...params, content: 'hello', cookie: { userid: '123', token: 'test' } },
      async (config) => {
        calls.push(config);
        return {
          status: 200,
          body: { status: 1, childrenid: 'resolved-id', list: [{ special_child_name: 'song' }] },
        };
      },
    );
...
test('album and playlist send directly with known id and name; blank content never reaches network', async () => {
  for (const name of ['comment_album_send', 'comment_playlist_send']) {
    const handler = require(`../server/module/${name}.js`);
    const calls = [];
    await handler({ id: '42', name: 'resource', content: 'hello', cookie: {} }, async (config) => {
      calls.push(config);
      return {};
    });
  ...
  for (const name of [
    'comment_music_send',
    'comment_album_send',
    'comment_playlist_send',
    'song_barrage_send',
    'video_barrage_send',
  ]) {
    const result = await require(`../server/module/${name}.js`)({ content: '  ' }, () =>
...
test('unresolved barrage hash does not trigger a send', async () => {
  for (const name of ['song_barrage_send', 'video_barrage_send']) {
    let count = 0;
    const result = await require(`../server/module/${name}.js`)(
      { hash: 'unknown', content: 'hello' },
      async () => {
        count++;
...
test('verification only resolves on an explicit successful business response', async () => {
  const output = buildSync({
    ...
    platform: 'node',
    write: false,
    external: ['vue', '@/utils/logger'],
  }).outputFiles[0].text;
  const module = { exports: {} };
  new Function('require', 'module', 'exports', output)(
    (name) => (name === 'vue' ? { reactive: (value) => value } : { error() {} }),
    module,
    module.exports,
  );
  const api = module.exports;
  const pending = api.requestKugouVerification('test-event', async (path) =>
    path === '/get/verify/info'
      ? { status: 1, data: { v_type: 23 } }
      : { status: 0, err_code: 30791 },
  );
...
test('floor UI keeps root tid separate from the selected reply pid across all pools', async () => {
  ...
  for (const resourceType of ['music', 'album', 'playlist']) {
    ...
    await api.sendFloorComment({
      root,
      target: { id: 'child', tid: 'root-tid', userName: 'Alice', content: 'original' },
      content: 'reply',
      resourceType,
    });
  }
...
test('floor module sends the correct pool, root and quoted target in a POST', async () => {
  ...
  for (const [resource_type, code] of [
    ['song', 'fc4be23b4e972707f36b8a828a93ba8a'],
    ['album', '94f1792ced1df89aa68a7939eaf2efca'],
    ['playlist', 'ca53b96fe5a1d9c22d71c8f522ef7c4f'],
  ]) {
    const calls = [];
    await handler(
      {
...
test('client rejects oversized barrage and comment bodies before making requests', async () => {
  ...
  await assert.rejects(
    api.sendComment({ type: 'song-barrage', hash: 'ABC' }, '😀'.repeat(101)),
    /100/,
  );
  await assert.rejects(api.sendComment({ type: 'music', id: '42' }, '字'.repeat(201)), /200/);
  await assert.rejects(
    api.sendFloorComment({
      root: {},
      target: {},
