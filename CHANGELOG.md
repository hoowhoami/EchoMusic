...
## [2.2.9-beta.7] - 2026-07-22

### 新增

- 新增插件播放展示状态 API
- 新增无缝播放开关，默认开启；播放临近结束时提前解析并预加载下一首，自动切歌时复用原生输出链路减少停顿
...
- 优化 `echo-ffmpeg-player` 的seek逻辑
- 优化 `echo-ffmpeg-player` 的独占模式
- 优化 `echo-ffmpeg-player` 的事件循环
- 优化桌面歌词切换动画速度，提升逐字歌词出现时的跟手感
- 优化页面歌词、桌面歌词与迷你播放器歌词的时间轴同步，统一以播放引擎进度为锚点并减少逐字歌词抢拍
- 将输出设备保护收敛为基于原生设备事件的“设备断开时暂停”开关，默认关闭，可手动开启以避免耳机断开后突然外放
- 优化播放器状态机
- 优化切歌交互响应
- 优化 MKV 人声/伴奏分轨播放链路，改为后台探测音轨并通过结构化播放源选择人声或伴奏
- 优化 EQ 与空间音效处理链，增加 EQ 自动增益余量，降低高增益设置导致削波失真的概率
- 优化空间音效强度调整逻辑，调整 Dry/Wet 混合比例时不再重复加载 IR 文件
- 空间音效默认强度由 40% 调整为 15%，减少 HiFi / 厅堂类 IR 的回音和拖尾感
