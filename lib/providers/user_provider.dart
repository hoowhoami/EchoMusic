...
class UserProvider with ChangeNotifier {
  ...
  List<Map<String, dynamic>> get createdPlaylists => _userPlaylists
      .where((p) => p['list_create_userid'] == _user?.userid)
      .toList();

  List<Map<String, dynamic>> get favoritedPlaylists => _userPlaylists
      .where((p) => p['list_create_userid'] != _user?.userid)
      .toList();
  ...
  void _updatePlaylistSongCount(dynamic listId, int delta) {
    final index = _userPlaylists.indexWhere((p) {
      final id =
          (p['listid'] ?? p['specialid'] ?? p['list_create_gid'] ?? p['gid'])
              ?.toString();
  ...
  Future<void> fetchLikedSongs({int? listid, String? gid}) async {
    final targetGid =
        gid ?? (likedPlaylistId is String ? likedPlaylistId : null);
    final targetListid =
        listid ?? (likedPlaylistId is int ? likedPlaylistId : null);
  ...
  Future<Map<String, dynamic>> login(
    ...
    final response = await MusicApi.loginCellphone(
      mobile,
      code,
      userid: userid,
    );
  ...
  Future<void> _autoClaimVipIfEnabled() async {
    final autoReceive =
        _persistenceProvider?.settings['autoReceiveVip'] as bool? ?? false;
    if (!autoReceive) {
    ...
    LoggerService.i(
      '[AutoVIP] 开始自动领取 VIP，当前状态：TVIP=$_isTvipClaimedToday，SVIP=$_isSvipClaimedToday',
    );
  ...
  Future<void> fetchUserPlaylists() async {
    ...
    if (response['status'] == 1) {
      ...
      if (likedIndex == -1) {
        likedIndex = _userPlaylists.indexWhere(
          (p) => p['name']?.toString().contains('喜欢') ?? false,
        );
      }
      ...
      if (likedIndex != -1) {
        ...
        _likedPlaylistId =
            liked['list_create_gid'] ?? liked['gid'] ?? liked['listid'];

        // Persist
        if (_user != null &&
            _user?.extendsInfo?['likedPlaylistId'] != _likedPlaylistId) {
  ...
  Future<void> fetchUserDetails() async {
    ...
    if (details != null && _user != null) {
      _user = _user!.copyWith(
        ...
        extendsInfo: {..._user!.extendsInfo ?? {}, 'detail': details},
      );
      await _persistenceProvider?.setUserInfo(_user!.toJson());
      notifyListeners();
    }
  ...
  Future<bool> unfavoritePlaylist(int playlistId, {String? globalId}) async {
    ...
    final id = playlistId.toString();
    final gid = globalId?.toString();
    final p = _userPlaylists.firstWhere((p) {
      final localId = (p['listid'] ?? p['specialid'])?.toString();
      final originalId = p['list_create_gid']?.toString();
      final originalListid = p['list_create_listid']?.toString();
  ...
  Future<int> addSongsToPlaylist(dynamic listId, List<Song> songs) async {
    ...
    final data = songs
        .map((s) => "${s.name}|${s.hash}|${s.albumId ?? 0}|${s.mixSongId}")
        .join(',');
  ...
  Future<bool> removeSongFromPlaylist(dynamic listId, Song song) async {
    ...
    final success = await MusicApi.deletePlaylistTrack(
      numericId,
      fileId.toString(),
    );
    if (success) {
  ...
  bool isCreatedPlaylist(dynamic listId) {
    ...
    return _userPlaylists.any(
      (p) =>
          (p['listid'] ?? p['specialid'] ?? p['list_create_gid'] ?? p['gid'])
                  .toString() ==
              id &&
          p['list_create_userid'] == _user?.userid,
    );
  }
}
