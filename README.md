...
- **插件扩展**：支持用户加载本地插件，自定义页面、侧边栏入口、设置项、播放器按钮、歌曲右键菜单与播放事件监听。
...
EchoMusic 支持本地插件扩展，插件可以注册页面、侧边栏、设置项、播放栏按钮、歌曲右键菜单，也可以通过 DOM selector 把 Vue 组件挂载到界面的任意位置。内置安全模式与故障自动恢复机制，插件出错时无需手动操作即可恢复正常。
